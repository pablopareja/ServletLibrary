 ...
 *
 */
public abstract class BasicLoginService extends HttpServlet {
    ...
    public final void init() {
        ...
        this.dbConnectionNeededFlag = defineDBConnectionNeededFlag();
    ...
    private final void servletLogic(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            boolean success = processLoginRequest(myRequest, connection);
            ...
            if (success) {
                ...
                session.setAttribute(SessionAttributes.PERMISSIONS_ATTRIBUTE, defineUserPermissions(myRequest, connection));

                if (DBConnection.SESSION_GUIDED_CONNECTIONS_FLAG && dbConnectionNeededFlag) {
                    //---->Storing a new connection for the Session<----
                    session.setAttribute(SessionAttributes.CONNECTION_ATTRIBUTE, DBConnection.getNewConnection());
                }
            ...
            if (!DBConnection.SESSION_GUIDED_CONNECTIONS_FLAG && connection != null) {
                try {
     ...
     */
    abstract protected void storeObjectsInSession(Request request, Connection connection, BasicSession session);
